     ...
     {filterSections?.map((section)=>(
      <div key={section?.id} className='space-y-3'>
        ...
        {section?.options?.map((option)=>(
          <Badge key={option.value} variant={
            section.currentValue === option.value ?'default':'outline'
          }
           className={`cursor-pointer px-3 py-1 ${section.currentValue === option.value?'bg-blue-100 hover:bg-blue-200 text-blue-900 border-blue-200':'bg-white hobver;bg-gray-100 hover:bg-gray-100 text-gray-700 '}`}
           onClick={()=>{
